...
-- create or replace procedure pgnetworks_staging(lower_bound bigint, upper_bound bigint)
-- language pspgsql
-- as $procedure$

do $$   -- 2025-01-16: avg 0.5 sec per 1000 vertices
declare
    -- process variables
    ...
    upper_bound bigint := 2595959187942176414;--2595912533134594922 --(100);--2595913045157100801 --(1000);--2595940121573209691 --(10000);-- 2595959187942176414 --(100000);    
    ...
    loop
        ...
        loop 
            ...
            select into closest
                   r.id as edge_id
                 , r.geom as edge_geom
              from osm.road_network_segmentized r
